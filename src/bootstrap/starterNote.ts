...
export const STARTER_NOTE_CONTENT =
	[
		...
		"- Problems or bugs: [open an issue on GitHub](https://github.com/MHElnour/tasknotes/issues)",
		"- Companion plugins:",
	...
	try {
		...
		if (existing) {
			...
			if (!(existingFile instanceof TFile)) {
				host.warn?.(
					`[TaskNotes][StarterNote] Starter note path exists but is not a file: ${normalizedPath}`
				);
			...
			file = await vault.create(normalizedPath, STARTER_NOTE_CONTENT);
		}
